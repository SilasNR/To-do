"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProjetoService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const projeto_entity_1 = require("../entity/projeto.entity");
let ProjetoService = class ProjetoService {
    constructor(projetoRepository) {
        this.projetoRepository = projetoRepository;
    }
    async findAll() {
        return await this.projetoRepository.find({ relations: ['projetos'] });
    }
    async findOne(id) {
        const projeto = await this.projetoRepository.findOne({
            where: { id_projeto: id },
            relations: ['projetos'],
        });
        if (!projeto) {
            throw new common_1.HttpException(`Projeto não encontrado.`, common_1.HttpStatus.NOT_FOUND);
        }
        return projeto;
    }
    async create(createProjetoDto) {
        try {
            return await this.projetoRepository.save(this.projetoRepository.create(createProjetoDto));
        }
        catch (error) {
            if (error.code === 'ER_DUP_ENTRY') {
                throw new common_1.HttpException('Projeto já registrado.', common_1.HttpStatus.BAD_REQUEST);
            }
            else {
                throw new common_1.HttpException('Erro ao criar o projeto. Tente novamente mais tarde', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }
    }
    async update(id, updateProjetoDto) {
        const result = await this.projetoRepository.update(id, updateProjetoDto);
        if (result.affected === 0) {
            throw new common_1.HttpException(`Projeto não econtrado.`, common_1.HttpStatus.NOT_FOUND);
        }
    }
    async delete(id) {
        const result = await this.projetoRepository.delete(id);
        if (result.affected === 0) {
            throw new common_1.HttpException(`Projeto não encontrado`, common_1.HttpStatus.NOT_FOUND);
        }
    }
};
exports.ProjetoService = ProjetoService;
exports.ProjetoService = ProjetoService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(projeto_entity_1.Projeto)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], ProjetoService);
//# sourceMappingURL=projeto.service.js.map